...
def mse_4d(y_true, y_pred, eps=1e-6):
    ...
    both tensors are [b, h, w, c] float
...
def build_unet_layers(input_tensor, filters = [32, 64, 128, 256, 512], factors = [2,2,2,2,2]):
    ...
    center = conv_block(net[f'encoder_pool{}'], filters[-1]*2)
    ...
    for i in range(1, levels+1):
        ...
        filt = filters[j]
        if i == 1:
            decoder = decoder_block(center, net[f'encoder_pool{j}'], filt, up_size = (factor, factor))
        else:
            decoder = decoder_block(decoder, net[f'encoder{j}'], filt)
...
def get_lstm_model(n_channels, n_time, activation = activations.relu(max_value = 2.0), optim, metrics, loss):
    ...
    activation = activation(dense_layer)
