...
#  fileList = [file.name for file in blobs if pred_image_base in file.name]
#  exportFiles = [file for file in fileList if pred_image_base in file]
...
#      jsonFile = f
  ...
  def toTupleImage(dic):
    if one_hot:
        ...
        stacked = tf.concat([oneHotList, normStack], axis=2)
    else:
